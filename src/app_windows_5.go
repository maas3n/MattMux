...
func showTextWindow(title,text string){hInstance,_,_:=procGetModuleHandleW.Call(0);className:=utf16Ptr("MattMuxTextWindowClass");cursor,_,_:=procLoadCursorW.Call(0,32512);bg,_,_:=procGetStockObject.Call(5);wc:=WNDCLASSEX{CbSize:uint32(unsafe.Sizeof(WNDCLASSEX{})),LpfnWndProc:syscall.NewCallback(textWindowProc),HInstance:hInstance,HCursor:cursor,HbrBackground:bg,LpszClassName:className};procRegisterClassExW.Call(uintptr(unsafe.Pointer(&wc)));hwnd,_,_:=procCreateWindowExW.Call(0,uintptr(unsafe.Pointer(className)),uintptr(unsafe.Pointer(utf16Ptr(title))),WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_THICKFRAME|WS_MINIMIZEBOX|WS_MAXIMIZEBOX|WS_VISIBLE,CW_USEDEFAULT,CW_USEDEFAULT,760,610,app.hwnd,0,hInstance,0);if hwnd==0{messageBox(app.hwnd,title,text,MB_OK|MB_ICONINFORMATION);return};edit,_,_:=procCreateWindowExW.Call(WS_EX_CLIENTEDGE,uintptr(unsafe.Pointer(utf16Ptr("EDIT"))),uintptr(unsafe.Pointer(utf16Ptr(text))),WS_CHILD|WS_VISIBLE|WS_VSCROLL|ES_MULTILINE|ES_AUTOVSCROLL|ES_READONLY,12,12,720,550,hwnd,1,hInstance,0);if app.monoFont!=0{procSendMessageW.Call(edit,WM_SETFONT,app.monoFont,1)};procShowWindow.Call(hwnd,SW_SHOW)}
func textWindowProc(hwnd uintptr,msg uint32,wParam,lParam uintptr) uintptr { switch msg{case 0x0005:var r struct{Left,Top,Right,Bottom int32};procGetClientRect.Call(hwnd,uintptr(unsafe.Pointer(&r)));getDlgItem:=user32.NewProc("GetDlgItem");child,_,_:=getDlgItem.Call(hwnd,1);if child!=0{procSetWindowPos.Call(child,0,12,12,uintptr(max32(100,r.Right-24)),uintptr(max32(100,r.Bottom-24)),0x0004)};return 0;case WM_CLOSE:procDestroyWindow.Call(hwnd);return 0};r,_,_:=procDefWindowProcW.Call(hwnd,uintptr(msg),wParam,lParam);return r }
